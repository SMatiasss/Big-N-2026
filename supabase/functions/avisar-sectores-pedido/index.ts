...
Deno.serve(async (req) => {
  ...
  try {
    ...
    etapa = 'buscar_items';
    const itemsResp = await fetch(`${url}/rest/v1/pedido_items?pedido_id=eq.${body.pedidoId}&select=sector`, { headers: apiHeaders });
    const items = await itemsResp.json();
    if (!items || !items.length) return responder({ ok: true, enviados: 0, mensaje: 'El pedido no tiene ítems.' });

    const sectores = [...new Set(items.map((i: any) => i.sector))];
    const rolesSectores = [];
    if (sectores.includes('cocina')) rolesSectores.push('cocinero');
    if (sectores.includes('bar')) rolesSectores.push('cantinero');

    if (rolesSectores.length === 0) return responder({ ok: true, enviados: 0, mensaje: 'No hay sectores válidos para avisar.' });

    etapa = 'buscar_empleados_sector';
    const empleadosResp = await fetch(`${url}/rest/v1/perfiles?rol=in.(${rolesSectores.join(',')})&activo=eq.true&estado=eq.aprobado&select=id`, { headers: apiHeaders });
    const empleados = await empleadosResp.json();
    if (!empleados || !empleados.length) return responder({ ok: true, enviados: 0, mensaje: 'No hay empleados de sector activos para recibir el aviso.' });
    
    const empleadosIds = empleados.map((m: any) => m.id);

    etapa = 'buscar_tokens';
    const tokensResp = await fetch(`${url}/rest/v1/push_tokens?usuario_id=in.(${empleadosIds.join(',')})&select=token,usuario_id`, { headers: apiHeaders });
    const tokens = await tokensResp.json();
    if (!tokens || !tokens.length) return responder({ ok: true, enviados: 0, mensaje: 'Los empleados de sector no tienen dispositivos registrados.' });
    ...
    const titulo = 'Pedido por preparar';
    const cuerpo = `La mesa ${numeroMesa} tiene un nuevo pedido para tu sector.`;
      ...
      const envio = await fetch(`https://fcm.googleapis.com/v1/projects/${cuenta.project_id}/messages:send`, {
        ...
        body: JSON.stringify({
          message: {
            ...
            data: { ruta: '/pedidos/cocina', tipo: 'preparacion_pedido' }, // A bit of a hack: ideally we'd send them to their own sector page.
            android: { priority: 'high', notification: { channel_id: 'pedidos-sectores' } },
          },
        }),
      });
      if (envio.ok) {
    ...
    const notificacionesInsert = empleadosIds.map((id: string) => ({
      destinatario_id: id,
      titulo,
      cuerpo,
      tipo: 'preparacion_pedido',
      datos: { ruta: '/pedidos/cocina' } 
    }));
    ...
    console.error('Error en avisar-sectores-pedido.', etapa, mensaje);
