-- La interfaz muestra cuatro sectores naranjas y cuatro verdes. La decisión
-- del servidor usa la misma correspondencia: par = premio, impar = sin premio.
...
begin
  ...
  if v_juego.descuento_pct = 10 then
    ...
    if p_eleccion is not null then
      raise exception 'La Rueda de la Fortuna no recibe una elección.' using errcode = '22023';
    end if;
    v_objetivo := floor(random() * 8)::integer;
    v_gano := mod(v_objetivo, 2) = 0;
  else
