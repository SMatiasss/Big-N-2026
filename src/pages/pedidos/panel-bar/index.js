...
export async function render(container) {
  ...
  const actualizacion = crearActualizacionHu11(raiz, async vigente => {
    ...
    for (const pedido of pedidos) {
      ...
      // Punto 18: el bar avisa que terminó su parte. Si con esto el pedido
      ...
      // estado en el servidor, así que si todavía falta la cocina no manda nada.
      const btnListo = tarjeta.querySelector('.btn-aceptar');
      btnListo.onclick = async () => {
        btnListo.disabled = true;
        try {
          await marcarSectorListo(pedido.id, 'bar');
          try {
